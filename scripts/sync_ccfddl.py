...
# Every area directory upstream publishes, mirroring the CCF catalogue at
# https://ccf.atom.im/. Display names come from the upstream types.yml and are
# written out to _data/areas.yml so the filter UI stays in sync automatically.
...
# Upstream's English area names are full CCF category strings, too long for a
# filter chip; these are the short labels shown in the UI.
AREA_LABELS = {
...
MAX_UPCOMING = 3
CCF_TAGS = {"A": "CCF-A", "B": "CCF-B", "C": "CCF-C"}
...
def tags_for(conf: dict) -> list[str]:
    ...
    tags.append(CCF_TAGS.get(str(rank.get("ccf")), "CCF-N"))
    core = CORE_TAGS.get(str(rank.get("core")))
    if core:
        tags.append(core)
    return tags
...
def write_areas(types_yaml: str) -> list[dict]:
    ...
    header = (
        ...
        "# CCF area codes, mirroring https://ccf.atom.im/\n"
    )
...
def write_ical(records: list[dict], path: pathlib.Path, stamp: dt.datetime, name: str) -> None:
    lines = [
        "BEGIN:VCALENDAR",
        "VERSION:2.0",
        "PRODID:-//ccf-conference-deadlines//EN",
        "CALSCALE:GREGORIAN",
        "METHOD:PUBLISH",
    ...
    for record in records:
        for index, deadline in enumerate(record["deadlines"]):
            moment = dt.datetime.fromisoformat(deadline["date"]).astimezone(dt.timezone.utc)
            title = f"{record['name']} {record['year']} deadline"
            if deadline.get("comment"):
                title += f" ({deadline['comment']})"
            uid = f"{record['name']}-{record['year']}-{index}".lower()
            ...
            lines += [
                "BEGIN:VEVENT",
                f"UID:{uid}@ccf-conference-deadlines",
                f"DTSTAMP:{dtstamp}",
                f"DTSTART:{moment.strftime('%Y%m%dT%H%M%SZ')}",
                f"DTEND:{moment.strftime('%Y%m%dT%H%M%SZ')}",
...
def main() -> int:
    ...
    header = (
        ...
        f"# Last synced: {now.strftime('%Y-%m-%d %H:%M:%S')} UTC\n"
    )
    ...
    write_ical(records, ICAL_DIR / "deadlines-all.ics", stamp, "CCF Deadlines - All areas")
    ...
    for area in AREAS:
        ...
        write_ical(
            ...
            stamp,
            f"CCF Deadlines - {labels.get(area, area)}",
        )
